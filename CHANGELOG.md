...
## [1.3.20](https://github.com/neveltyc/VCD_ANALYZER/releases/tag/v1.3.20) - 2026-07-20

Two correctness fixes found by review, plus documentation and validation cleanup. Every fix is locked with a regression test in the existing harnesses (`test_parser_direct_coverage.py`, `test_commands_direct_coverage.py`, `test_summary_begin_boundary.py`, `test_parser_optimizations.py`); the full suite passes.
...
- **`info` `t_max` collapsed to `t_min` when the final timestamp was followed by more than 4 MiB of value changes.** The backward tail scan read 64 KiB..4 MiB windows and gave up without a hit, with a comment claiming a >4 MiB timestamp-less tail is malformed — it is not: the IEEE 1364 `simulation_command` grammar places no bound on the value_change run after a `simulation_time`. A legal file `#0 ... #100 <8 MiB of changes>` reported `0s ~ 0s` and made `search` (implicit end) error with "begin time ... is after the last event at 0s". The tail scan is now a reverse token walk in 4 MiB windows: the first top-level `#<digits>` met from EOF is the last timestamp, so the scan stops at the first hit and never degrades. Reverse section handling is the exact dual of the forward skip-to-$end walk (a '$end' met in reverse order enters a region whose body is skipped until the region's opening keyword), so `$comment`/`$vcdclose` bodies are never read as top level — matching the forward parser and `iter_events` — and `#<digits>` identifier_codes are excluded exactly as the parser does. Chunks are split on raw ASCII whitespace so multi-byte comment bodies can never merge tokens across a window boundary. A ~8 MiB fixture locks the behavior.
...
Correctness pass on free-format VCD (IEEE 1364-2005 permits several declarations or timestamps per physical line, and value_change identifiers that clash with structural tokens). Every fix below was reproduced with a minimal fixture and locked with a regression test in `verify/test_freeformat_1_3_19.py`; the previously-green suite still passes.
